...
{$ENDIF}
    ...
    protected
        ...
        FFDSet: fd_set;
    //
        class function FDSelect(AReadSet, AWriteSet, AExceptSet: Pfd_set; const ATimeout: Integer): Integer;
    ...
    public
        ...
        class function Select(AReadList: TIdSocketList; AWriteList: TIdSocketList; AExceptList: TIdSocketList; const ATimeout: Integer = IdTimeoutInfinite): Boolean; override;
        function SelectRead(const ATimeout: Integer = IdTimeoutInfinite): Boolean; override;
        function SelectReadList(var VSocketList: TIdSocketList; const ATimeout: Integer = IdTimeoutInfinite): Boolean; override;
    end;


    TIdStackVCLPosix = class(TIdStackBSDBase)
    protected
        procedure WriteChecksumIPv6(s: TIdStackSocketHandle; var VBuffer: TIdBytes; const AOffset: Integer; const AIP: String; const APort: TIdPort);
        function GetLastError: Integer;
        procedure SetLastError(const AError: Integer);
        function HostByName(const AHostName: string; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION): string; override;
        function ReadHostName: string; override;
        function WSCloseSocket(ASocket: TIdStackSocketHandle): Integer; override;
        function WSRecv(ASocket: TIdStackSocketHandle; var ABuffer; const ABufferLength, AFlags: Integer): Integer; override;
...
{$IFNDEF VCL_XE3_OR_ABOVE}
        ...
        procedure WSSetSocketOption(ASocket: TIdStackSocketHandle; ALevel: TIdSocketOptionLevel; AOptName: TIdSocketOption; const AOptVal; const AOptLen: Integer); override;
{$ENDIF}
    public
        ...
        procedure Bind(ASocket: TIdStackSocketHandle; const AIP: string; const APort: TIdPort; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
        procedure Connect(const ASocket: TIdStackSocketHandle; const AIP: string; const APort: TIdPort; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
        ...
        procedure GetPeerName(ASocket: TIdStackSocketHandle; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion); override;
        procedure GetSocketName(ASocket: TIdStackSocketHandle; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion); override;
        ...
        function RecvFrom(const ASocket: TIdStackSocketHandle; var VBuffer; const ALength, AFlags: Integer; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion): Integer; override;
        function ReceiveMsg(ASocket: TIdStackSocketHandle; var VBuffer: TIdBytes; APkt: TIdPacketInfo): UInt32; override;
        procedure WSSendTo(ASocket: TIdStackSocketHandle; const ABuffer; const ABufferLength, AFlags: Integer; const AIP: string; const APort: TIdPort; AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
        function WSSocket(AFamily: Integer; AStruct: TIdSocketType; AProtocol: Integer; const ANonBlocking: Boolean = False): TIdStackSocketHandle; override;
...
{$IFDEF VCL_XE3_OR_ABOVE}
        ...
        procedure SetSocketOption(ASocket: TIdStackSocketHandle; ALevel: TIdSocketOptionLevel; AOptName: TIdSocketOption; const AOptVal; const AOptLen: Integer); override;
{$ENDIF}
...
// Note that this should be called
    // IMMEDIATELY before you do a SendTo because the Local IPv6 address might change
        ...
        procedure WriteChecksum(s: TIdStackSocketHandle; var VBuffer: TIdBytes; const AOffset: Integer; const AIP: String; const APort: TIdPort; const AIPVersion: TIdIPVersion = ID_DEFAULT_IP_VERSION); override;
...
begin
    Lock;
    try
        if not __FD_ISSET(AHandle, FFDSet) then
        begin
            if Count >= FD_SETSIZE then
            begin
...
function TIdSocketListVCLPosix.ContainsSocket(AHandle: TIdStackSocketHandle): Boolean;
begin
    Lock;
    try
...
class function TIdSocketListVCLPosix.FDSelect(AReadSet, AWriteSet, AExceptSet: Pfd_set; const ATimeout: Integer): Integer;
var
    LTime: TimeVal;
    LTimePtr: PTimeVal;
begin
...
begin
    Lock;
    try
        if __FD_ISSET(AHandle, FFDSet) then
        begin
            Dec(FCount);
...
class function TIdSocketListVCLPosix.Select(AReadList, AWriteList, AExceptList: TIdSocketList; const ATimeout: Integer): Boolean;
...
function TIdSocketListVCLPosix.SelectReadList(var VSocketList: TIdSocketList; const ATimeout: Integer): Boolean;
var
    LSet: fd_set;
begin
    Lock;
...
function TIdStackVCLPosix.Accept(ASocket: TIdStackSocketHandle; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion): TIdStackSocketHandle;
var
    LN: socklen_t;
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
...
procedure TIdStackVCLPosix.Bind(ASocket: TIdStackSocketHandle; const AIP: string; const APort: TIdPort; const AIPVersion: TIdIPVersion);
var
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
    LAddrIPv6: SockAddr_in6 absolute LAddrStore;
...
function TIdStackVCLPosix.CheckIPVersionSupport(const AIPVersion: TIdIPVersion): Boolean;
var
...
procedure TIdStackVCLPosix.Connect(const ASocket: TIdStackSocketHandle; const AIP: string; const APort: TIdPort; const AIPVersion: TIdIPVersion);
var
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
    LAddrIPv6: SockAddr_in6 absolute LAddrStore;
    ...
    case AIPVersion of
        Id_IPv4:
            begin
                ...
                begin
                    LTimeOut.tv_sec := (ConnectTimeout div 1000);
                    LTimeOut.tv_usec := ((ConnectTimeout mod 1000) + 125) * 1000;
                    SetSocketOption(ASocket, Id_SOL_SOCKET, Id_SO_SNDTIMEO, LTimeOut, SizeOf(LTimeOut));
                end;
                if ReadTimeout > 0 then
                begin
                    LTimeOut.tv_sec := (ReadTimeout div 1000);
                    LTimeOut.tv_usec := ((ReadTimeout mod 1000) + 125) * 1000;
                    SetSocketOption(ASocket, Id_SOL_SOCKET, Id_SO_RCVTIMEO, LTimeOut, SizeOf(LTimeOut));
                end;
...
procedure TIdStackVCLPosix.GetPeerName(ASocket: TIdStackSocketHandle; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion);
var
    i: socklen_t;
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
...
procedure TIdStackVCLPosix.GetSocketName(ASocket: TIdStackSocketHandle; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion);
var
    LiSize: socklen_t;
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
...
function TIdStackVCLPosix.HostByAddress(const AAddress: string; const AIPVersion: TIdIPVersion): string;
var
    LiSize: socklen_t;
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
...
{$ELSE}
         LHostName
{$ENDIF}, NI_MAXHOST, nil, 0, NI_NAMEREQD);
    if LRet <> 0 then
    begin
        if LRet = EAI_SYSTEM then
        begin
            RaiseLastOSError;
        end
        else
...
{$ELSE}
         LHostName
{$ENDIF}, '0', LHints, LAddrInfo) = 0 then
    begin
...
function TIdStackVCLPosix.HostByName(const AHostName: string; const AIPVersion: TIdIPVersion): string;
var
    LAddrInfo: pAddrInfo;
    LHints: AddrInfo;
    LRetVal: Integer;
...
function TIdStackVCLPosix.IOControl(const s: TIdStackSocketHandle; const cmd: UInt32; var arg: UInt32): Integer;
begin
    Result := ioctl(s, cmd, @arg);
end;


procedure TIdStackVCLPosix.Listen(ASocket: TIdStackSocketHandle; ABackLog: Integer);
begin
...
function TIdStackVCLPosix.ReceiveMsg(ASocket: TIdStackSocketHandle; var VBuffer: TIdBytes; APkt: TIdPacketInfo): UInt32;
var
    LSize: socklen_t;
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
    ...
    LControl: TIdBytes;
    LCurCmsg: Pcmsghdr;
                    // for iterating through the control buffer
    LByte: PByte;
...
function TIdStackVCLPosix.RecvFrom(const ASocket: TIdStackSocketHandle; var VBuffer; const ALength, AFlags: Integer; var VIP: string; var VPort: TIdPort; var VIPVersion: TIdIPVersion): Integer;
var
    LiSize: socklen_t;
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
...
procedure TIdStackVCLPosix.SetBlocking(ASocket: TIdStackSocketHandle; const ABlocking: Boolean);
var
    LFlags: Integer;
begin
...
procedure TIdStackVCLPosix.{$IFDEF VCL_XE3_OR_ABOVE}GetSocketOption{$ELSE}WSGetSocketOption{$ENDIF}
     (ASocket: TIdStackSocketHandle; ALevel: TIdSocketOptionLevel; AOptName: TIdSocketOption; var AOptVal; var AOptLen: Integer);
var
    LLen: socklen_t;
begin
    LLen := AOptLen;
...
procedure TIdStackVCLPosix.{$IFDEF VCL_XE3_OR_ABOVE}SetSocketOption{$ELSE}WSSetSocketOption{$ENDIF}
     (ASocket: TIdStackSocketHandle; ALevel: TIdSocketOptionLevel; AOptName: TIdSocketOption; const AOptVal; const AOptLen: Integer);
begin
...
begin
  ...
  // have often the same value and so FPC might report a range error
    Result := (AResult = Id_WSAEAGAIN) or (AResult = Id_WSAEWOULDBLOCK) or (AResult = Id_WSAEINPROGRESS);
end;


procedure TIdStackVCLPosix.WriteChecksum(s: TIdStackSocketHandle; var VBuffer: TIdBytes; const AOffset: Integer; const AIP: String; const APort: TIdPort; const AIPVersion: TIdIPVersion);
begin
    case AIPVersion of
        Id_IPv4:
            CopyTIdUInt16(HostToLittleEndian(CalcCheckSum(VBuffer)), VBuffer, AOffset);
        Id_IPv6:
...
procedure TIdStackVCLPosix.WriteChecksumIPv6(s: TIdStackSocketHandle; var VBuffer: TIdBytes; const AOffset: Integer; const AIP: String; const APort: TIdPort);
begin
...
function TIdStackVCLPosix.WSRecv(ASocket: TIdStackSocketHandle; var ABuffer; const ABufferLength, AFlags: Integer): Integer;
begin
...
function TIdStackVCLPosix.WSSend(ASocket: TIdStackSocketHandle; const ABuffer; const ABufferLength, AFlags: Integer): Integer;
begin
...
procedure TIdStackVCLPosix.WSSendTo(ASocket: TIdStackSocketHandle; const ABuffer; const ABufferLength, AFlags: Integer; const AIP: string; const APort: TIdPort; AIPVersion: TIdIPVersion);
var
    LAddrStore: sockaddr_storage;
    LAddrIPv4: SockAddr_In absolute LAddrStore;
    LAddrIPv6: SockAddr_in6 absolute LAddrStore;
...
begin
    ...
    LBytesSent := Posix.SysSocket.sendto(ASocket, ABuffer, ABufferLength, AFlags or Id_MSG_NOSIGNAL, LAddr, LiSize);
    if LBytesSent = Id_SOCKET_ERROR then
    begin

        if WSGetLastError() = Id_WSAEMSGSIZE then
        begin
    ...
    else if LBytesSent <> ABufferLength then
    begin
...
function TIdStackVCLPosix.WSShutdown(ASocket: TIdStackSocketHandle; AHow: Integer): Integer;
begin
    Result := Posix.SysSocket.shutdown(ASocket, AHow);
end;


function TIdStackVCLPosix.WSSocket(AFamily: Integer; AStruct: TIdSocketType; AProtocol: Integer; const ANonBlocking: Boolean = False): TIdStackSocketHandle;
var
    LFlags: Integer;
begin
