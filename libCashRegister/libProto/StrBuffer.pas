...
type

  {
    ...
    public
        ...
        procedure Clear;
        procedure AddS(const aValue: TBytes { AnsiString } ); overload;
    ...
    TSegment = record
        ...
        Size: Integer;
        Count: Integer;
        Data: array [0 .. 0] of Byte { AnsiChar };
    end;
  ...
  { add memory done by segments }
    ...
    public
        ...
        procedure AddB(const aValue: Byte { AnsiChar } ); overload;
        procedure AddS(const aValue: TBytes { AnsiString } ); overload;
        procedure AddP(const aValue: PByte; aCnt: Integer); overload;
        function GetText: TBytes { AnsiString };
        function GetCount: Integer;
        property Text: TBytes { AnsiString } read GetText;
    end;
...
procedure TStrBuffer.AddS(const aValue: TBytes { AnsiString } );
var
    cnt, delta: Integer;
begin
    cnt := Length(aValue);
    ...
    System.Move(aValue, PByte(FBuff + FCount)^, cnt);
    Inc(FCount, cnt);
end;
...
function TStrBuffer.GetText: TBytes { AnsiString };
begin
    SetLength(result, FCount);
    System.Move(FBuff^, result, FCount);
end;
...
var
    segment: PSegment;
begin
    segment := AllocMem(aSize + SizeOf(TSegment) - SizeOf(Byte { AnsiChar } ));
    with segment^ do
    begin
        Next := nil;
        Size := aSize;
...
function TSegmentBuffer.GetText: TBytes { AnsiString };
var
    p: PByte;
    segment: PSegment;
    len: Integer;
...
procedure TSegmentBuffer.AddP(const aValue: PByte; aCnt: Integer);
var
    p: PByte;
    tmp: Integer;
begin
...
procedure TSegmentBuffer.AddS(const aValue: TBytes { AnsiString } );
var
    len: Integer;
begin
    len := Length(aValue);
...
procedure TSegmentBuffer.AddB(const aValue: Byte { AnsiChar } );
begin
    AddP(@aValue, 1);
end;
