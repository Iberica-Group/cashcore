...
type
    ...
    public const
        ...
        tag_turnover             = 2;
        tag_sum                  = 3;
        tag_turnover_without_tax = 4;

    strict private
        Foperation: TOperationTypeEnum;
        Fturnover: TMoney;
        Fsum: TMoney;
        Fturnover_without_tax: TMoney;
    ...
    public
        ...
        property turnover_without_tax: TMoney read Fturnover_without_tax;
...
begin
    for var item in operationsList do
        item.Free;
    inherited;
end;
...
begin
    ...
    for var item in sectionsList do
        item.Free;

    for var item in operationsList do
        item.Free;

    for var item in discountsList do
        item.Free;

    for var item in markupsList do
        item.Free;

    for var item in total_resultList do
        item.Free;

    for var item in taxesList do
        item.Free;

    for var item in start_shift_non_nullable_sumsList do
        item.Free;

    for var item in ticket_operationsList do
        item.Free;

    for var item in money_placementsList do
        item.Free;

    for var item in non_nullable_sumsList do
        item.Free;
...
begin
    ...
    try
        ...
        if FieldHasValue[tag_shift_number] then
            ProtoBuf.writeUInt32(tag_shift_number, shift_number);
...
(* procedure TMoneyPlacementRequest.Setoperator(Tag: Integer; const Value: TOperator);
begin
    Foperator := Value;
    FieldHasValue[Tag] := True;
end; *)
...
procedure TMoneyPlacementRequest.Setprinted_document_number(Tag: Integer; const Value: UInt64);
begin
...
begin
    ...
    Fsum := TMoney.Create;
    RegisterRequiredField(tag_sum);
    Fturnover_without_tax := TMoney.Create;
end;
...
begin
    ...
    try
        case FieldNumber of
            ...
            tag_turnover_without_tax:
                begin
                    tmpBuf := ProtoBuf.ReadSubProtoBufInput;
                    Fturnover_without_tax.LoadFromBuf(tmpBuf);
                end
            else
                Result := False;
        end;
...
begin
    ...
    try
        ...
        if FieldHasValue[tag_sum] then
            SaveMessageFieldToBuf(Fsum, tag_sum, tmpBuf, ProtoBuf);
        if FieldHasValue[tag_turnover_without_tax] then
            SaveMessageFieldToBuf(Fturnover_without_tax, tag_turnover_without_tax, tmpBuf, ProtoBuf);
    finally
        tmpBuf.Free
    end;
end;
...
begin
    for var item in annulled_operationsList do
        item.Free;
    inherited;
end;
...
procedure TCloseShiftRequest.Setprinted_document_number(Tag: Integer; const Value: UInt64);
begin
    Fprinted_document_number := Value;
    FieldHasValue[Tag] := True;
end;
