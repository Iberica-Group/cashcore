...
{$ENDIF} FTransparentProxy: TIdCustomTransparentProxy;
...
{$IFDEF DARWIN}
    if (LBinding.IP <> '') or (LBinding.Port <> 0) or ((LBinding.ClientPortMin <> 0) and (LBinding.ClientPortMax <> 0)) then
    begin
        LBinding.Bind;
    end;
{$ELSE}
    GStack.ConnectTimeout := ConnectTimeout;
    GStack.ReadTimeout := ReadTimeout;
    LBinding.Bind;
{$ENDIF}
...
begin
    if Assigned(FOnBeforeBind) then
    begin
        FOnBeforeBind(self);
    end;
end;
...
begin
    if Assigned(FOnSocketAllocated) then
    begin
...
begin
    Result := Host;
    if (Port <> DefaultPort) and (Port > 0) then
    begin
...
function TIdIOHandlerSocket.WriteFile(const AFile: String; AEnableTransferFile: Boolean): Int64;
...
{$ENDIF}
     ...
     { and (Intercept = nil) } and AEnableTransferFile then
            begin
...
begin
    ...
    // under ARC, all weak references to a freed object get nil'ed automatically
        ...
        begin
            if not Assigned(AProxy.Owner) then
            begin
                if Assigned(LTransparentProxy) and (not FImplicitTransparentProxy) then
                begin
...
{$ENDIF}
                ...
                if Assigned(LTransparentProxy) and (LTransparentProxy.ClassType <> LClass) then
                begin
                    FTransparentProxy := nil;
                    FImplicitTransparentProxy := False;
                    IdDisposeAndNil(LTransparentProxy);
                end;
                if not Assigned(LTransparentProxy) then
                begin
            ...
            begin
                ...
                begin
                    if FImplicitTransparentProxy then
                    begin
                        FTransparentProxy := nil;
                        FImplicitTransparentProxy := False;
                        IdDisposeAndNil(LTransparentProxy);
...
{$ENDIF}
        ...
        else if Assigned(LTransparentProxy) then
        begin
            if FImplicitTransparentProxy then
            begin
                FTransparentProxy := nil;
...
begin
    if (Operation = opRemove) and (AComponent = FTransparentProxy) then
    begin
        FTransparentProxy := nil;
        FImplicitTransparentProxy := False;
    end;
